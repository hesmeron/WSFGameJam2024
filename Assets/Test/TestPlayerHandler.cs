    ...
    void Update()
    {
        ...
        if (interactingObject == null)
        {
            ...
            if (hit.collider != null)
            {
                ...
                if(pointedObject.TryGetComponent<TestElement>(out element)){
                    highlightedObject = pointedObject;
                    element.HighLightThis();
                }
            ...
            {
                if (highlightedObject.TryGetComponent<TestSocket>(out socket))
                {
                    TestSocketHighlight.sh.UnHighlight();
                }
                if (highlightedObject.TryGetComponent<TestElement>(out element))
                {
                    element.UnhighlightThis();
                }

            }

            if (Input.GetMouseButtonDown(0) && highlightedObject.TryGetComponent<TestSocket>(out socket))
            {
                interactingObject = highlightedObject;
                TestSocketHighlight.sh.UnHighlight();
                socket = pointedObject.GetComponent<TestSocket>();
                element = socket.GetElement();
            ...
            if(Input.GetMouseButtonDown(0) && highlightedObject.TryGetComponent<TestElement>(out element))
            {
                interactingObject = highlightedObject;
                Debug.Log(highlightedObject.ToString());
                Debug.Log(interactingObject.ToString());
                rotating = true;
                interactionElement = element;
        ...
        else //INTERAKCJA
        {
            if (rotating)
            {
                interactingObject.transform.Rotate(0f, 15f * Input.GetAxis("Mouse X"), 0f);
                interactingObject.transform.Rotate(15f * Input.GetAxis("Mouse Y"), 0f, 0f);
                TestElement element;

                if (Input.GetMouseButtonUp(0) && highlightedObject.TryGetComponent<TestElement>(out element))
                {
